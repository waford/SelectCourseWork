...
allocuvm(pde_t *pgdir, uint oldsz, uint newsz, uint flags)
{
  ...
  for(; a < newsz; a += PGSIZE){
	if(flags & PTE_NOLAZY) {
		...
		if(mappages(pgdir, (char*)a, PGSIZE, V2P(zero_page), flags & ~PTE_W) < 0){
			myproc()->killed=1;
		    //cprintf("Map to zero page failed\n");
		}
	}
  }
  return newsz;
...
copyuvm(pde_t *pgdir, uint sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    if((pte = walkpgdir(pgdir, (void *) i, 0)) == 0)
   		//Checks that every page table entry exists
	  panic("copyuvm: pte should exist");
    if(!(*pte & PTE_P))
	  //Checks that every page is present
      panic("copyuvm: page not present");
	
	//Disable write on page
	*pte &= ~PTE_W;
	
	//Invalidate copied page that we just updated
	

	invlpg((void *) i);

	//Copies the address of the pte and flags
	pa = PTE_ADDR(*pte);
    flags = PTE_FLAGS(*pte);
	

//	cprintf("Physical Address %x has flags %x\n", pa, flags);	

	//Increase page reference counter
	acquire(&pageref_lock);
	pageref[PGIND(pa)]++;
	release(&pageref_lock);
//    Mapes the new pte to the given physical memory
	if(mappages(d, (void*)i, PGSIZE, pa, flags) < 0) {
//   		cprintf("Failed to map 0x%x to pa 0x$x\n",d, pa);
		 goto bad;
    }

	//	cprintf("Mapped physical page %x to va %x\n", pa, i);	
  }
/*  for(i = 0; i < sz; i += PGSIZE) {
  	pte = walkpgdir(d, (void *) i, 0);
	pa = PTE_ADDR(*pte);
	flags = PTE_FLAGS(*pte);
	cprintf("PTE: %x, PA: %x, Flags: %x\n", pte, pa, flags);
  }
  while(1);*/
  return d;

bad:
  //Frees any allocated memory for this process because there 
  //is not enough to allocate the enitre virtual memory
  //freevm(d);
  return 0;
}
